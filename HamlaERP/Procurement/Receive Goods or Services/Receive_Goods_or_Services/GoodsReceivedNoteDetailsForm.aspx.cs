...
public partial class Procurement_Receive_Goods_or_Services_Receive_Goods_or_Services_GoodsReceivedNoteDetailsForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
                ...
                {
                    txtGoodsReceivedNoteGuid.ReadOnly = true;
                    txtPOGuid.ReadOnly = true;
                    txtAssetGuid.ReadOnly = true;
                    txtLocationGuid.ReadOnly = true;
                    txtQuantityInvoiced.ReadOnly = true;
                    txtPreviouslyShipped.ReadOnly = true;
                    txtPreviouslyInvoiced.ReadOnly = true;
                    txtUnitCost.ReadOnly = true;
                    txtExtendedCost.ReadOnly = true;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            goodsReceivedNoteDetailsFormUI.tbl_PODetailsId = txtPODetails.Text;
    ...
    #region PO Details Search
    protected void btnPODetailsSearch_Click(object sender, EventArgs e)
    {
        btnHtmlPODetailsSearch.Visible = false;
        btnHtmlPODetailsClose.Visible = true;
        SearchPODetailsList();
    }
    protected void btnClearPODetailsSearch_Click(object sender, EventArgs e)
    {
        BindPODetailsList();
        gvPODetailsSearch.Visible = true;
        btnHtmlPODetailsSearch.Visible = true;
        btnHtmlPODetailsClose.Visible = false;
        txtPODetailsSearch.Text = "";
        txtPODetailsSearch.Focus();
    }
    protected void btnPODetailsRefresh_Click(object sender, EventArgs e)
    {
        System.Threading.Thread.Sleep(1000);
        BindPODetailsList();
    }
    #endregion PO Details Search
    ...
    private void FillForm(GoodsReceivedNoteDetailsFormUI goodsReceivedNoteDetailsFormUI)
    {
        try
        {
            ...
            if (dtb.Rows.Count > 0)
            {
                txtPODetails.Text = dtb.Rows[0]["tbl_GoodsReceivedNoteId"].ToString();
    ...
    #region PO Detials Search
    private void SearchPODetailsList()
    {
        try
        {
            PODetailsListUI pODetailsListUI = new PODetailsListUI();
            pODetailsListUI.Search = txtPODetailsSearch.Text;
            DataTable dtb = pODetailsListBAL.GetPODetailsListBySearchParameters(pODetailsListUI);
            if (dtb.Rows.Count > 0 && dtb != null)
            {
                gvPODetailsSearch.DataSource = dtb;
                gvPODetailsSearch.DataBind();
                divPODetailsSearchError.Visible = false;
                gvPODetailsSearch.Visible = true;
            }
            else
            {
                divPODetailsSearchError.Visible = true;
                lblPODetailsSearchError.Text = Resources.GlobalResource.msgNoRecordFound;
                gvPODetailsSearch.Visible = false;
            }
        }
        catch (Exception exp)
        {
            logExcpUIobj.MethodName = "SearchPODetailsList()";
            logExcpUIobj.ResourceName = "Procurement_Receive_Goods_or_Services_Receive_Goods_or_Services_GoodsReceivedNoteDetailsForm.CS";
            logExcpUIobj.RecordId = "All";
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);

            log.Error("[Procurement_Receive_Goods_or_Services_Receive_Goods_or_Services_GoodsReceivedNoteDetailsForm : SearchPODetailsList] An error occured in the processing of Record. Details : [" + exp.ToString() + "]");
        }
    }
    private void BindPODetailsList()
    {
        try
        {
        ...
        {
            logExcpUIobj.MethodName = "BindPODetailsList()";
            logExcpUIobj.ResourceName = "Procurement_Receive_Goods_or_Services_Receive_Goods_or_Services_GoodsReceivedNoteDetailsForm.CS";
            logExcpUIobj.RecordId = "All";
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
