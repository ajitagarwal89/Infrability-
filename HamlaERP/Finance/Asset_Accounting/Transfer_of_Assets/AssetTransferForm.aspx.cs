...
public partial class Finance_Asset__Accounting_Transfer_of_Assets_AssetTransferForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            if (Request.QueryString["AssetTransferId"] != null || Request.QueryString["recordId"] != null)
            {
                assetTransferFormUI.Tbl_AssetTransferId = (Request.QueryString["AssetTransferId"] != null ? Request.QueryString["AssetTransferId"] : Request.QueryString["recordId"]);
            ...
            {
                assetTransferFormUI.InvoiceType = invoiceTypeInvoice;
                GetSerialNumber(assetTransferFormUI);
                BindDocumentTypeDropDownList();
    ...
    {
        string Form = "~/Finance/Asset_Accounting/Transfer_of_Assets/AssetTransferForm.aspx";
        string recordId = Request.QueryString["AssetTransferId"];
    ...
    protected void btnPost_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            assetTransferFormUI.Tbl_OrganizationId = SessionContext.OrganizationId;
            assetTransferFormUI.Tbl_AssetTransferId = Request.QueryString["AssetTransferId"];
            assetTransferFormUI.IsPosted = isPosted;

            if (assetTransferFormBAL.UpdatePostingAssetTransfer(assetTransferFormUI) == 1)
            {
                divSuccess.Visible = true;
        ...
        catch (Exception exp)
        {
            ...
            log.Error("[Finance_Asset__Accounting_Transfer_of_Assets_AssetTransferForm : btnPost_Click] An error occured in the processing of Record Id : " + assetTransferFormUI.Tbl_AssetTransferId + ".  Details : [" + exp.ToString() + "]");
        }
    }
    #region Batch Search
    ...
    private void GetSerialNumber(AssetTransferFormUI assetTransferFormUI)
    {
        try
        {
            DataTable dtb = assetTransferFormBAL.GetSerialNumber(assetTransferFormUI);
            if (dtb.Rows.Count > 0)
            {
                txtNumber.Text = dtb.Rows[0][0].ToString();
        ...
        catch (Exception exp)
        {
            ...
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
            log.Error("[Finance_Asset__Accounting_Transfer_of_Assets_AssetTransferForm : GetSerialNumber] An error occured in the processing of Record Details : [" + exp.ToString() + "]");
        }
    }
