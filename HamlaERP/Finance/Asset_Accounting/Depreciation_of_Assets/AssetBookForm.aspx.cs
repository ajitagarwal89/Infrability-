...
public partial class Asset_AssetBookForm : PageBase
{
    ...
    protected override void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            if (Request.QueryString["AssetBookId"] != null || Request.QueryString["recordId"] != null)
            {
                assetBookFormUI.Tbl_AssetBookId = (Request.QueryString["AssetBookId"] != null ? Request.QueryString["AssetBookId"] : Request.QueryString["recordId"]);
            ...
            {
                assetBookFormUI.InvoiceType = invoiceTypeInvoice;
                GetSerialNumber(assetBookFormUI);
                BindStatusDropDown();
                BindAveragingConventionDropDown();
                BindDepreciationMethodDropDown();
    ...
    {
        string Form = "~/Finance/Asset_Accounting/Capitalization_of_Assets/AssetBookForm.aspx";
        string recordId = Request.QueryString["AssetBookId"];
    ...
    private void GetSerialNumber(AssetBookFormUI assetBookFormUI)
    {
        try
        {
            DataTable dtb = assetBookFormBAL.GetSerialNumber(assetBookFormUI);
            if (dtb.Rows.Count > 0)
            {
                txtAssetBookCode.Text = dtb.Rows[0][0].ToString();
        ...
        catch (Exception exp)
        {
            ...
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
            log.Error("[Asset_AssetBookForm : GetSerialNumber] An error occured in the processing of Record Details : [" + exp.ToString() + "]");
        }
    }
