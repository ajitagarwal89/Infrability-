...
public partial class Assets_AssetPurchase_AssetPurchaseForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                assetPurchaseFormUI.InvoiceType = invoiceTypeInvoice;
                GetSerialNumber(assetPurchaseFormUI);
                BindReceiptTypeDropDownList();
    ...
    {
        string Form = "~/Finance/Asset_Accounting/Fixed_Asset_Purchase/AssetPurchaseForm.aspx";
        string recordId = Request.QueryString["tbl_AssetPurchaseId"];
    ...
    protected void btnPost_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            assetPurchaseFormUI.Tbl_OrganizationId = SessionContext.OrganizationId;
            assetPurchaseFormUI.Tbl_AssetPurchaseId = Request.QueryString["AssetPurchaseId"];
            assetPurchaseFormUI.IsPosted = isPosted;

            if (assetPurchaseFormBAL.UpdatePostingAssetPurchase(assetPurchaseFormUI) == 1)
            {
                divSuccess.Visible = true;
        ...
        catch (Exception exp)
        {
            ...
            log.Error("[Assets_AssetPurchase_AssetPurchaseForm : btnPost_Click] An error occured in the processing of Record Id : " + assetPurchaseFormUI.Tbl_AssetPurchaseId + ".  Details : [" + exp.ToString() + "]");
        }
    }
    ...
    private void GetSerialNumber(AssetPurchaseFormUI assetPurchaseFormUI)
    {
        try
        {
            DataTable dtb = assetPurchaseFormBAL.GetSerialNumber(assetPurchaseFormUI);
            if (dtb.Rows.Count > 0)
            {
        ...
        catch (Exception exp)
        {
            ...
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
            log.Error("[Assets_AssetPurchase_AssetPurchaseForm : GetSerialNumber] An error occured in the processing of Record Details : [" + exp.ToString() + "]");
        }
    }
