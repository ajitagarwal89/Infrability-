...
public partial class Finance_Accounts_Receivable_Customer__Invoice__With_Sales_Order_CustomerInvoiceForm : PageBase
{
    ...
    protected override void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            if (Request.QueryString["CustomerInvoiceId"] != null || Request.QueryString["recordId"] != null)
            {
                customerInvoiceFormUI.Tbl_CustomerInvoiceId = (Request.QueryString["CustomerInvoiceId"] != null ? Request.QueryString["CustomerInvoiceId"] : Request.QueryString["recordId"]);
            ...
            else
            {
                ...
                customerInvoiceFormUI.InvoiceType = invoiceTypeInvoice;
                GetSerialNumber(customerInvoiceFormUI);
                BindDocumentStatusList();
                BindInvoiceTypeDropDownList();
    ...
    {
        string Form = "~/Finance/Accounts_Receivable/Customer_Invoice_(With_Sales_Order)/CustomerInvoiceForm.aspx";
        string recordId = Request.QueryString["CustomerInvoiceId"];
    ...
    protected void btnPost_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            customerInvoiceFormUI.Tbl_OrganizationId = SessionContext.OrganizationId;
            customerInvoiceFormUI.Tbl_CustomerInvoiceId = Request.QueryString["CustomerInvoiceId"];
            customerInvoiceFormUI.IsPosted = isPosted;

            if (customerInvoiceFormBAL.UpdatePostingCustomerInvoice(customerInvoiceFormUI) == 1)
            {
                divSuccess.Visible = true;
        ...
        catch (Exception exp)
        {
            ...
            logExcpUIobj.ResourceName = "Finance_Accounts_Receivable_Customer__Invoice__With_Sales_Order_CustomerInvoiceForm.CS";
            logExcpUIobj.RecordId = customerInvoiceFormUI.Tbl_CustomerInvoiceId;
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);

            log.Error("[Finance_Accounts_Receivable_Customer__Invoice__With_Sales_Order_CustomerInvoiceForm : btnPost_Click] An error occured in the processing of Record Id : " + customerInvoiceFormUI.Tbl_CustomerInvoiceId + ".  Details : [" + exp.ToString() + "]");
        }
    }
    ...
    private void GetSerialNumber(CustomerInvoiceFormUI customerInvoiceFormUI)
    {
        try
        {
            DataTable dtb = customerInvoiceFormBAL.GetSerialNumber(customerInvoiceFormUI);
            if (dtb.Rows.Count > 0)
            {
        ...
        catch (Exception exp)
        {
            ...
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
            log.Error("[Finance_Accounts_Receivable_Customer__Invoice__With_Sales_Order_CustomerInvoiceForm : GetSerialNumber] An error occured in the processing of Record Details : [" + exp.ToString() + "]");
        }
    }
    ...
    private void SearchBatchList()
    {
        ...
        catch (Exception exp)
        {
            ...
            log.Error("[Finance_Accounts_Receivable_Customer__Invoice__With_Sales_Order_CustomerInvoiceForm : SearchBatchList] An error occured in the processing of Record. Details : [" + exp.ToString() + "]");
        }
    ...
    private void BindBatchList()
    {
        ...
        catch (Exception exp)
        {
            ...
            log.Error("[Finance_Accounts_Receivable_Customer__Invoice__With_Sales_Order_CustomerInvoiceForm : BindBatchList] An error occured in the processing of Record. Details : [" + exp.ToString() + "]");
        }
    }
