...
public partial class Finance_Accounts_Payable_PO_PODetailsForm : PageBase 
{
    #region Data Members
        ...
        { if (Request.QueryString["POId"] != null)
            {
                ...
                txtPOIdGuid.Text = Request.QueryString["POId"];
    ...
    //            pODetailsFormUI.CreatedBy = SessionContext.UserGuid;
    //            pODetailsFormUI.Tbl_OrganizationId = SessionContext.OrganizationId;
    //            pODetailsFormUI.Tbl_POId = txtAssetIdGuid.Text;
    //            pODetailsFormUI.Tbl_AssetId_FixedAsset = txtAssetIdGuid.Text;
    //            pODetailsFormUI.UOM = txtUOM.Text;
    //            pODetailsFormUI.Description = txtDescription.Text;
    //            pODetailsFormUI.Tbl_LocationId = txtLocationGuid.Text;
    ...
    {
        Response.Redirect("~/Finance/Accounts_Payable/PO/POForm.aspx");
    }
    ...
    private void BindPODetaild()
    {
        ...
        {
            txtPOId.Text = dtb.Rows[0]["PONumber"].ToString();
    ...
    private void AddDefaultFirstRecord()
    {
      ...
      dt.TableName = "AddProduct";
        ...
        dt.Columns.Add(new DataColumn("QuantityOrdered", typeof(string)));
        dt.Columns.Add(new DataColumn("UnitCost", typeof(string)));
        dt.Columns.Add(new DataColumn("ExtendedCost", typeof(string)));
        dr = dt.NewRow();
        dt.Rows.Add(dr);
       
      ViewState["AddProduct"] = dt;
       gvAdd.DataSource = dt;
        gvAdd.DataBind();
    }
    protected void btnAddProduct_Click(object sender, EventArgs e)
    {
        if (ViewState["AddProduct"] != null)
        {
            DataTable dt = (DataTable)ViewState["AddProduct"];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    dr = dt.NewRow();
                    dr["PONumber"] = txtPOId.Text;
                    dr["Asset"] = txtAssetId.Text;
                    dr["UOM"] = txtUOM.Text;
                    dr["ProductName"] = txtDescription.Text;
                    dr["Location"] = txtLocation.Text;
                    dr["Quantity"] = Convert.ToDecimal(txtQuantityOrdered.Text);
                    dr["QuantityOrdered"] = Convert.ToDecimal(txtQuantityOrdered.Text);
                    dr["UnitCost"] = Convert.ToDecimal(txtUnitCost.Text);
                    dr["ExtendedCost"] = Convert.ToDecimal(txtExtendedCost.Text);
                    dt.Rows.Add(dr);
                    dt.AcceptChanges();
                    gvAdd.DataSource = dt;
                    clear();
