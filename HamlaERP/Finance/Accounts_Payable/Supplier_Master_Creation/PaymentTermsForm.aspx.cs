...
public partial class Finance_Accounts_Payable_Supplier_Master_Creation_PaymentTermsForm : PageBase
{
       ...
       // PaymentTermsFormUI paymentTermsFormUI = new PaymentTermsFormUI();
        if (!Page.IsPostBack)
        {
           // BindDiscountTypeDropDownList();
            if (Request.QueryString["PaymentTermsId"] != null || Request.QueryString["recordId"] != null)
            {
                paymentTermsFormUI.Tbl_PaymentTermsId = (Request.QueryString["PaymentTermsId"] != null ? Request.QueryString["PaymentTermsId"] : Request.QueryString["recordId"]);
                BindDiscountTypeDropDownList();
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            paymentTermsFormUI.DueInDays = int.Parse(txtDueInDays.Text);
            paymentTermsFormUI.DiscountInDays = int.Parse(txtDiscountInDays.Text);
            paymentTermsFormUI.Tbl_PaymentTermsId = Request.QueryString["PaymentTermsId"];
            if (chkSalesOrPurchase.Checked)
            {
                paymentTermsFormUI.SalesOrPurchase = true;
            }
    ...
    {
        string Form = "~/Finance/Accounts_Payable/Supplier_Master_Creation/PaymentTermsForm.aspx";
        string recordId = Request.QueryString["PaymentTermsId"];
    ...
    private void BindDiscountTypeDropDownList()
    {
        try
        {
            ...
            if (dtb.Rows.Count > 0)
            {
                ...
                ddlOpt_DiscountType.DataBind();
                ddlOpt_DiscountType.Items.Insert(0, new ListItem(Resources.GlobalResource.msgSelectRecord, "0"));
                ddlOpt_DiscountType.SelectedIndex = 0;
            }
            else
            {
