...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
    <script src="../../infra_ui/js/Asset/AssetPurchaseDetails_JS/AssetPurchaseDetailsList.js"></script>
</asp:Content>
                            ...
                            <ContentTemplate>
                                ...
                                <asp:DataGrid runat="server" ClientIDMode="Static" Font-Size="10pt" ID="gvData" AutoGenerateColumns="false" Width="100%" GridLines="None" BorderStyle="None" CssClass="table table-hover">
                                    <Columns>
                                        ...
                                        <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, AssetPurchaseDetails_htPONumber%>" DataField="tbl_PONumber"></asp:BoundColumn>
                                        ...
                                        <asp:TemplateColumn HeaderImageUrl="~/images/eye.png" HeaderStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                <button id="lnkView_<%# Container.DataSetIndex + 1 %>" type="button" data-toggle="modal" data-target="#divBasicModal" data-keyboard="false" data-backdrop="static" class="btn btn-default btn-sm btn-block" onclick="setDetails('<%#Eval("tbl_PONumber")%>','<%#Eval("tbl_Asset")%>','<%#Eval("UOM")%>','<%#Eval("tbl_Location")%>','<%#Eval("QuantityOrdered")%>','<%#Eval("QuantityShipped")%>','<%#Eval("QuantityInvoiced")%>','<%#Eval("PreviouslyShipped")%>','<%#Eval("PreviouslyInvoiced")%>','<%#Eval("UnitCost")%>','<%#Eval("ExtendedCost")%>','lnkview_<%# Container.DataSetIndex + 1 %>');">
                                                    <asp:Literal runat="server" ID="ltrView" Text="<%$ Resources:GlobalResource, ltrView%>"></asp:Literal></button>
                                                <button id="lnkViewNextPrev_<%# Container.DataSetIndex + 1 %>" style="display: none;" type="button" class="btn btn-default btn-sm" onclick="setDetails('<%#Eval("tbl_PONumber")%>','<%#Eval("tbl_Asset")%>','<%#Eval("UOM")%>','<%#Eval("tbl_Location")%>','<%#Eval("QuantityOrdered")%>','<%#Eval("QuantityShipped")%>','<%#Eval("QuantityInvoiced")%>','<%#Eval("PreviouslyShipped")%>','<%#Eval("PreviouslyInvoiced")%>','<%#Eval("UnitCost")%>','<%#Eval("ExtendedCost")%>','lnkview_<%# Container.DataSetIndex + 1 %>');"></button>
                                            </ItemTemplate>
                                        </asp:TemplateColumn>
