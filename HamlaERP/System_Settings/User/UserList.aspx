<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="UserList.aspx.cs" Inherits="System_Settings_UserList" %>
    ...
    <div id="content" class="ui-content ui-content-aside-overlay">
        ...
        <div class="ui-content-body">
            <div class="ui-container">
                <div class="row">
                    <h3 class="hepad">
                        <asp:Literal runat="server" ID="ltrSearch" Text="<%$ Resources:GlobalResource, User_ltrInformation %>"></asp:Literal></h3>
                            ...
                            <ContentTemplate>
                                ...
                                <asp:DataGrid runat="server" ClientIDMode="Static" Font-Size="10pt" ID="gvData" AutoGenerateColumns="false" Width="100%" GridLines="None" BorderStyle="None" CssClass="table table-hover">
                                    <Columns>
                                        ...
                                        <asp:BoundColumn DataField="tbl_UserId" Visible="false"></asp:BoundColumn>
										 <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htEmployeeCode%>" DataField="EmployeeCode"></asp:BoundColumn>
                                        <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htFullName%>" DataField="FullName"></asp:BoundColumn>                                    
										<asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htDOB%>" DataField="DOB"></asp:BoundColumn>
                                        <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htDepartment%>" DataField="Department"></asp:BoundColumn>
                                        <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htDesignation%>" DataField="Designation"></asp:BoundColumn>
										 <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htEmail%>" DataField="Email"></asp:BoundColumn>
										 <asp:BoundColumn HeaderText="<%$ Resources:GlobalResource, User_htMobile%>" DataField="Mobile"></asp:BoundColumn>
										
										         <asp:TemplateColumn HeaderImageUrl="~/images/eye.png" HeaderStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                <button id="lnkView_<%# Container.DataSetIndex + 1 %>" type="button" data-toggle="modal" data-target="#divBasicModal" data-keyboard="false" data-backdrop="static" class="btn btn-default btn-sm btn-block" onclick="SetDetails('<%#Eval("EmployeeCode")%>','<%#Eval("FullName")%>','<%#Eval("DOB")%>','<%#Eval("PermanentAddress")%>','<%#Eval("CorrespondanceAddress")%>','<%#Eval("City")%>','<%#Eval("State")%>','<%#Eval("PostalCode")%>','<%#Eval("PhoneNo")%>','<%#Eval("Mobile")%>','<%#Eval("Department")%>','<%#Eval("DateOfJoining")%>','<%#Eval("Designation")%>','<%#Eval("UserName")%>','lnkview_<%# Container.DataSetIndex + 1 %>');">
                                                    <asp:Literal runat="server" ID="ltrView" Text="<%$ Resources:GlobalResource, ltrView%>"></asp:Literal></button>
                                                <button id="lnkViewNextPrev_<%# Container.DataSetIndex + 1 %>" style="display: none;" type="button" class="btn btn-default btn-sm" onclick="SetDetails('<%#Eval("EmployeeCode")%>','<%#Eval("FullName")%>','<%#Eval("DOB")%>','<%#Eval("PermanentAddress")%>','<%#Eval("CorrespondanceAddress")%>','<%#Eval("City")%>','<%#Eval("State")%>','<%#Eval("PostalCode")%>','<%#Eval("PhoneNo")%>','<%#Eval("Mobile")%>','<%#Eval("Department")%>','<%#Eval("DateOfJoining")%>','<%#Eval("Designation")%>','<%#Eval("UserName")%>','lnkview_<%# Container.DataSetIndex + 1 %>');"></button>
                                            </ItemTemplate>
                                        </asp:TemplateColumn>

                                        <asp:TemplateColumn HeaderImageUrl="~/images/edit.png" HeaderStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                <asp:LinkButton runat="server" ID="gvColLnkBtn" class="btn btn-default btn-sm btn-block" OnClick="gvColLnkBtn_Click" CommandArgument='<%#Eval("tbl_userId") %>'>
