...
/// </summary>
public class PayablesListDAL : IRequiresSessionState
{
    ...
    public DataTable GetPayablesListById(PayablesListUI payablesListUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Payables", payablesListUI.Tbl_PayablesId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public DataTable GetPayablesListBySearchParameters(PayablesListUI payablesListUI)
    {
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
                dtbl = ds.Tables[0];
            string recordId = dtbl.Rows[0]["tbl_PayablesId"].ToString();
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                string selectedValue;
                selectedValue = JsonConvert.SerializeObject(dtbl);
                audit_IUD.WebServiceSelectInsert("tbl_Payables", recordId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int DeletePayables(PayablesListUI payablesListUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Payables", payablesListUI.Tbl_PayablesId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
