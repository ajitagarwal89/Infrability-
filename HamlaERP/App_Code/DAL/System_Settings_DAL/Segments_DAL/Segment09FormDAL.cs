...
/// </summary>
public class Segment09FormDAL : IRequiresSessionState
{
    ...
    public DataTable GetSegment09ListById(Segment09FormUI segment09FormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Segment09", segment09FormUI.Tbl_Segment09Id, selectedValue);
            }
    ...
    public int AddSegment09(Segment09FormUI segment09FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_Segment09Id", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_Segment09Id"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_Segment09Id"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_Segment09";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment09FormUI);
                    audit_IUD.WebServiceInsert(segment09FormUI.Tbl_OrganizationId, tableName, RecoredID, segment09FormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment09FormUI);
                    audit_IUD.WebServiceUpdate(segment09FormUI.Tbl_OrganizationId, "tbl_Segment09", segment09FormUI.Tbl_Segment09Id, segment09FormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteSegment09(Segment09FormUI segment09FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Segment09", segment09FormUI.Tbl_Segment09Id);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
