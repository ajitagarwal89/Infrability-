...
/// </summary>
public class Segment08FormDAL : IRequiresSessionState
{
    ...
    public DataTable GetSegment08ListById(Segment08FormUI segment08FormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Segment08", segment08FormUI.Tbl_Segment08Id, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddSegment08(Segment08FormUI segment08FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_Segment08Id", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_Segment08Id"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_Segment08Id"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_Segment08";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment08FormUI);
                    audit_IUD.WebServiceInsert(segment08FormUI.Tbl_OrganizationId, tableName, RecoredID, segment08FormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment08FormUI);
                    audit_IUD.WebServiceUpdate(segment08FormUI.Tbl_OrganizationId, "tbl_Segment08", segment08FormUI.Tbl_Segment08Id, segment08FormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteSegment08(Segment08FormUI segment08FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Segment08", segment08FormUI.Tbl_Segment08Id);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
