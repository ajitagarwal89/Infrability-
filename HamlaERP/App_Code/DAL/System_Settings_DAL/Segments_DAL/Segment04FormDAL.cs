...
/// </summary>
public class Segment04FormDAL : IRequiresSessionState
{
    ...
    public DataTable GetSegment04ListById(Segment04FormUI segment04FormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Segment04", segment04FormUI.Tbl_Segment04Id, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddSegment04(Segment04FormUI segment04FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_Segment04Id", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_Segment04Id"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_Segment04Id"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_Segment04";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment04FormUI);
                    audit_IUD.WebServiceInsert(segment04FormUI.Tbl_OrganizationId, tableName, RecoredID, segment04FormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment04FormUI);
                    audit_IUD.WebServiceUpdate(segment04FormUI.Tbl_OrganizationId, "tbl_Segment04", segment04FormUI.Tbl_Segment04Id, segment04FormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteSegment04(Segment04FormUI segment04FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Segment04", segment04FormUI.Tbl_Segment04Id);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
