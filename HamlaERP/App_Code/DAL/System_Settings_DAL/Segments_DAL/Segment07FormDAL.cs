...
/// </summary>
public class Segment07FormDAL : IRequiresSessionState
{
    ...
    public DataTable GetSegment07ListById(Segment07FormUI segment07FormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Segment07", segment07FormUI.Tbl_Segment07Id, selectedValue);
            }
    ...
    public int AddSegment07(Segment07FormUI segment07FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_Segment07Id", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_Segment07Id"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_Segment07Id"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_Segment07";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment07FormUI);
                    audit_IUD.WebServiceInsert(segment07FormUI.Tbl_OrganizationId, tableName, RecoredID, segment07FormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(segment07FormUI);
                    audit_IUD.WebServiceUpdate(segment07FormUI.Tbl_OrganizationId, "tbl_Segment07", segment07FormUI.Tbl_Segment07Id, segment07FormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteSegment07(Segment07FormUI segment07FormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Segment07", segment07FormUI.Tbl_Segment07Id);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
