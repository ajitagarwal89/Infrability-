...
/// </summary>
public class CardFormDAL : IRequiresSessionState
{
    ...
    public DataTable GetCardListById(CardFormUI cardFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Card", cardFormUI.Tbl_CardId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddCard(CardFormUI cardFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_CardId", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_CardId"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_CardId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_Card";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(cardFormUI);
                    audit_IUD.WebServiceInsert(cardFormUI.Tbl_OrganizationId, tableName, RecoredID, cardFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(cardFormUI);
                    audit_IUD.WebServiceUpdate(cardFormUI.Tbl_OrganizationId, "tbl_Card", cardFormUI.Tbl_CardId, cardFormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteCard(CardFormUI cardFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Card", cardFormUI.Tbl_CardId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
