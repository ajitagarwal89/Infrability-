...
/// </summary>
public class OrganizationTypeFormDAL : IRequiresSessionState
{
    ...
    public DataTable GetOrganizationTypeListById(OrganizationTypeFormUI organizationTypeFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_OrganizationType", organizationTypeFormUI.Tbl_OrganizationTypeId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddOrganizationType(OrganizationTypeFormUI organizationTypeFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@Tbl_OrganizationTypeId", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@Tbl_OrganizationTypeId"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@Tbl_OrganizationTypeId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_OrganizationType";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(organizationTypeFormUI);
                    audit_IUD.WebServiceInsert(SessionContext.OrganizationId, tableName, RecoredID, organizationTypeFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(organizationTypeFormUI);
                    audit_IUD.WebServiceUpdate(SessionContext.OrganizationId, "tbl_OrganizationType", organizationTypeFormUI.Tbl_OrganizationTypeId, organizationTypeFormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteOrganizationType(OrganizationTypeFormUI organizationTypeFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_OrganizationType", organizationTypeFormUI.Tbl_OrganizationTypeId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
