...
/// </summary>
public class BudgetDetailsListDAL : IRequiresSessionState
{
    ...
    public DataTable GetBudgetDetailsListById(BudgetDetailsListUI budgetDetailsListUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_BudgetDetails", budgetDetailsListUI.Tbl_BudgetDetailsId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int DeleteBudgetDetails(BudgetDetailsListUI budgetDetailsListUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_BudgetDetails", budgetDetailsListUI.Tbl_BudgetDetailsId);
                }
    ...
    public DataTable GetBudgetDetailsListBySearchParameters(BudgetDetailsListUI budgetDetailsListUI)
    {
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
                dtbl = ds.Tables[0];
            string recordId = dtbl.Rows[0]["tbl_BudgetDetailsId"].ToString();
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                string selectedValue;
                selectedValue = JsonConvert.SerializeObject(dtbl);
                audit_IUD.WebServiceSelectInsert("tbl_BudgetDetails", recordId, selectedValue);
            }
        }
        catch (Exception exp)
        {
