...
/// </summary>
public class BatchListDAL : IRequiresSessionState
{
    ...
    public DataTable GetBatchListById(BatchListUI batchListUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_Batch", batchListUI.Tbl_BatchId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int DeleteBatch(BatchListUI batchListUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_Batch", batchListUI.Tbl_BatchId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public DataTable GetBatchListBySearchParameters(BatchListUI batchListUI)
    {
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
                dtbl = ds.Tables[0];
            string recordId = dtbl.Rows[0]["tbl_BatchId"].ToString();
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                string selectedValue;
                selectedValue = JsonConvert.SerializeObject(dtbl);
                audit_IUD.WebServiceSelectInsert("tbl_Batch", recordId, selectedValue);
            }
        }
        catch (Exception exp)
        {
