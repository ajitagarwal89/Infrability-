...
/// </summary>
public class PaymentToSupplierDistributionFormDAL : IRequiresSessionState
{
    ...
    public DataTable GetPaymentToSupplierDistributionListById(PaymentToSupplierDistributionFormUI PaymentToSupplierDistributionFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_PaymentToSupplierDistribution", PaymentToSupplierDistributionFormUI.Tbl_PaymentToSupplierDistributionId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddPaymentToSupplierDistribution(PaymentToSupplierDistributionFormUI PaymentToSupplierDistributionFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters["@tbl_OrganizationIdCorrespondence"].Value = PaymentToSupplierDistributionFormUI.CorrespondenceCompanyId;

                sqlCmd.Parameters.Add("@Tbl_PaymentToSupplierDistributionId", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@Tbl_PaymentToSupplierDistributionId"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@Tbl_PaymentToSupplierDistributionId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_PaymentToSupplierDistribution";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(PaymentToSupplierDistributionFormUI);
                    audit_IUD.WebServiceInsert(PaymentToSupplierDistributionFormUI.Tbl_OrganizationId, tableName, RecoredID, PaymentToSupplierDistributionFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(PaymentToSupplierDistributionFormUI);
                    audit_IUD.WebServiceUpdate(PaymentToSupplierDistributionFormUI.Tbl_OrganizationId, "tbl_PaymentToSupplierDistribution", PaymentToSupplierDistributionFormUI.Tbl_PaymentToSupplierDistributionId, PaymentToSupplierDistributionFormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeletePaymentToSupplierDistribution(PaymentToSupplierDistributionFormUI PaymentToSupplierDistributionFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_PaymentToSupplierDistribution", PaymentToSupplierDistributionFormUI.Tbl_PaymentToSupplierDistributionId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
