...
/// </summary>
public class DownPaymentFromCustomerFormDAL : IRequiresSessionState
{
    string connectionString = string.Empty;
    int commandTimeout = 0;
    DateTime FiscalPeriodDate;
    ...
    public DataTable GetDownPaymentFromCustomerListById(DownPaymentFromCustomerFormUI downPaymentFromCustomerFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_DownPaymentFromCustomer", downPaymentFromCustomerFormUI.Tbl_DownPaymentFromCustomerId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddDownPaymentFromCustomer(DownPaymentFromCustomerFormUI downPaymentFromCustomerFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters["@Opt_PayablesType"].Value = downPaymentFromCustomerFormUI.Opt_PayablesType;
                ...
                sqlCmd.Parameters.Add("@tbl_DownPaymentFromCustomerId", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_DownPaymentFromCustomerId"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_DownPaymentFromCustomerId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_DownPaymentFromCustomer";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(downPaymentFromCustomerFormUI);
                    audit_IUD.WebServiceInsert(downPaymentFromCustomerFormUI.Tbl_OrganizationId, tableName, RecoredID, downPaymentFromCustomerFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(downPaymentFromCustomerFormUI);
                    audit_IUD.WebServiceUpdate(downPaymentFromCustomerFormUI.Tbl_OrganizationId, "tbl_DownPaymentFromCustomer", downPaymentFromCustomerFormUI.Tbl_DownPaymentFromCustomerId, downPaymentFromCustomerFormUI.ModifiedBy, newValue);
                }
    ...
    public int DeleteDownPaymentFromCustomer(DownPaymentFromCustomerFormUI downPaymentFromCustomerFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_DownPaymentFromCustomer", downPaymentFromCustomerFormUI.Tbl_DownPaymentFromCustomerId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int UpdatePostingDownPaymentFromCustomer(DownPaymentFromCustomerFormUI downPaymentFromCustomerFormUI)
    {
        ...
        try
        {
            ...
            {
                SupportCon.Open();
                SqlCommand sqlCmd = new SqlCommand("SP_DownPaymentFromCustomer_UpdatePosting", SupportCon);
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.CommandTimeout = commandTimeout;
        ...
        {
            logExcpUIobj.MethodName = "UpdatePostingDownPaymentFromCustomer()";
            logExcpUIobj.ResourceName = "DownPaymentFromCustomerFormDAL.CS";
            logExcpUIobj.RecordId = downPaymentFromCustomerFormUI.Tbl_DownPaymentFromCustomerId;
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
            log.Error("[DownPaymentFromCustomerFormDAL : UpdatePostingDownPaymentFromCustomer] An error occured in the processing of Record Id : " + downPaymentFromCustomerFormUI.Tbl_DownPaymentFromCustomerId + ". Details : [" + exp.ToString() + "]");
        }
    ...
    public DataTable GetFiscalPeriod(DownPaymentFromCustomerFormUI downPaymentFromCustomerFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                SqlCommand sqlCmd = new SqlCommand("SP_FiscalPeriod", SupportCon);
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.CommandTimeout = commandTimeout;
        ...
        catch (Exception exp)
        {
            ...
            logExcpUIobj.RecordId = downPaymentFromCustomerFormUI.Tbl_OrganizationId;
            logExcpUIobj.ExceptionDetails = "Error Occured. System Generated Error is: " + exp.ToString();
            logExcpDALobj.SaveExceptionToDB(logExcpUIobj);
            log.Error("[DownPaymentFromCustomerListDAL : GetDownPaymentFromCustomerListById] An error occured in the processing of Record Id : " + downPaymentFromCustomerFormUI.Tbl_OrganizationId+ ". Details : [" + exp.ToString() + "]");
        }
        finally
        {
            ds.Dispose();
    ...
    public DataTable GetSerialNumber(DownPaymentFromCustomerFormUI downPaymentFromCustomerFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                SqlCommand sqlCmd = new SqlCommand("SP_DownPaymentFromCustomer_SerialNumber", SupportCon);
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.CommandTimeout = commandTimeout;
                sqlCmd.Parameters.Add("@RecordType", SqlDbType.TinyInt);
                sqlCmd.Parameters["@RecordType"].Value = downPaymentFromCustomerFormUI.InvoiceType;
        ...
        catch (Exception exp)
        {
            ...
            log.Error("[DownPaymentFromCustomerFormDAL : GetSerialNumber] An error occured in the processing of Record Id : " + downPaymentFromCustomerFormUI.Tbl_DownPaymentFromCustomerId + ". Details : [" + exp.ToString() + "]");
        }
        finally
        {
            ds.Dispose();
