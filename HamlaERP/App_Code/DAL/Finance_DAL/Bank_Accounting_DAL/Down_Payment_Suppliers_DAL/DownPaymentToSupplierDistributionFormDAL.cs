...
/// </summary>
public class DownPaymentToSupplierDistributionFormDAL : IRequiresSessionState
{
    ...
    public DataTable GetDownPaymentToSupplierDistributionListById(DownPaymentToSupplierDistributionFormUI downPaymentToSupplierDistributionFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_DownPaymentToSupplierDistribution", downPaymentToSupplierDistributionFormUI.Tbl_DownPaymentToSupplierDistributionId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddDownPaymentToSupplierDistribution(DownPaymentToSupplierDistributionFormUI downPaymentToSupplierDistributionFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters["@tbl_OrganizationIdCorrespondence"].Value = downPaymentToSupplierDistributionFormUI.CorrespondenceCompanyId;

                sqlCmd.Parameters.Add("@tbl_DownPaymentToSupplierDistributionId", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_DownPaymentToSupplierDistributionId"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_DownPaymentToSupplierDistributionId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_DownPaymentToSupplierDistribution";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(downPaymentToSupplierDistributionFormUI);
                    audit_IUD.WebServiceInsert(downPaymentToSupplierDistributionFormUI.Tbl_OrganizationId, tableName, RecoredID, downPaymentToSupplierDistributionFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(downPaymentToSupplierDistributionFormUI);
                    audit_IUD.WebServiceUpdate(downPaymentToSupplierDistributionFormUI.Tbl_OrganizationId, "tbl_DownPaymentToSupplierDistribution", downPaymentToSupplierDistributionFormUI.Tbl_DownPaymentToSupplierDistributionId, downPaymentToSupplierDistributionFormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeleteDownPaymentToSupplierDistribution(DownPaymentToSupplierDistributionFormUI downPaymentToSupplierDistributionFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_DownPaymentToSupplierDistribution", downPaymentToSupplierDistributionFormUI.Tbl_DownPaymentToSupplierDistributionId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
