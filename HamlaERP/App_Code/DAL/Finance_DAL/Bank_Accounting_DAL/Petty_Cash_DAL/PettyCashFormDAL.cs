...
/// </summary>
public class PettyCashFormDAL : IRequiresSessionState
{
    ...
    public DataTable GetPettyCashListById(PettyCashFormUI pettyCashFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_PettyCash", pettyCashFormUI.Tbl_PettyCashId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddPettyCash(PettyCashFormUI pettyCashFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_PettyCashId", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_PettyCashId"].Direction = ParameterDirection.Output;

                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_PettyCashId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_PettyCash";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(pettyCashFormUI);
                    audit_IUD.WebServiceInsert(pettyCashFormUI.Tbl_OrganizationId, tableName, RecoredID, pettyCashFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(pettyCashFormUI);
                    audit_IUD.WebServiceUpdate(pettyCashFormUI.Tbl_OrganizationId, "tbl_PettyCash", pettyCashFormUI.Tbl_PettyCashId, pettyCashFormUI.ModifiedBy, newValue);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
    ...
    public int DeletePettyCash(PettyCashFormUI pettyCashFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_PettyCash", pettyCashFormUI.Tbl_PettyCashId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
