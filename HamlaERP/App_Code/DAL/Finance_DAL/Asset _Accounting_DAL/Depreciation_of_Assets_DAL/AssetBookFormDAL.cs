...
/// </summary>
public class AssetBookFormDAL : IRequiresSessionState
{
    ...
    public DataTable GetAssetBookListById(AssetBookFormUI assetBookFormUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_AssetBook", assetBookFormUI.Tbl_AssetBookId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int AddAssetBook(AssetBookFormUI assetBookFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                sqlCmd.Parameters.Add("@tbl_AssetBookID", SqlDbType.UniqueIdentifier);
                sqlCmd.Parameters["@tbl_AssetBookID"].Direction = ParameterDirection.Output;


                result = sqlCmd.ExecuteNonQuery();

                string RecoredID = Convert.ToString(sqlCmd.Parameters["@tbl_AssetBookId"].Value);

                if (SessionContext.GlobalAuditEnabledStatus == true)
                {

                    string tableName = "tbl_AssetBook";

                    sqlCmd.Dispose();
                    SupportCon.Close();
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(assetBookFormUI);
                    audit_IUD.WebServiceInsert(assetBookFormUI.Tbl_OrganizationId, tableName, RecoredID, assetBookFormUI.CreatedBy, newValue);
                    return 1;
                }
                else
                {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    SerializeInputParameters obj = new SerializeInputParameters();
                    string newValue = obj.GetSerializedString(assetBookFormUI);
                    audit_IUD.WebServiceUpdate(assetBookFormUI.Tbl_OrganizationId, "tbl_AssetBook", assetBookFormUI.Tbl_AssetBookId, assetBookFormUI.ModifiedBy, newValue);
                }
    ...
    public int DeleteAssetBook(AssetBookFormUI assetBookFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_AssetBook", assetBookFormUI.Tbl_AssetBookId);
                }
    ...
    public DataTable GetSerialNumber(AssetBookFormUI assetBookFormUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                SqlCommand sqlCmd = new SqlCommand("SP_AssetBook_SerialNumber", SupportCon);
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.CommandTimeout = commandTimeout;
                sqlCmd.Parameters.Add("@RecordType", SqlDbType.TinyInt);
                sqlCmd.Parameters["@RecordType"].Value = assetBookFormUI.InvoiceType;
        ...
        catch (Exception exp)
        {
            ...
            log.Error("[AssetBookFormDAL : GetSerialNumber] An error occured in the processing of Record Id : " + assetBookFormUI.Tbl_AssetBookId + ". Details : [" + exp.ToString() + "]");
        }
        finally
        {
            ds.Dispose();
