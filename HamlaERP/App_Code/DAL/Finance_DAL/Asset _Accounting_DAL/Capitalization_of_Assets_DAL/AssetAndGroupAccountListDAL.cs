...
/// </summary>
public class AssetAndGroupAccountListDAL : IRequiresSessionState
{
    ...
    public DataTable GetAssetAndGroupAccountListById(AssetAndGroupAccountListUI assetAndGroupAccountListUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_AssetAndGroupAccount", assetAndGroupAccountListUI.Tbl_AssetAndGroupAccountId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public DataTable GetAssetAndGroupAccountListBySearchParameters(AssetAndGroupAccountListUI assetAndGroupAccountListUI)
    {
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
                dtbl = ds.Tables[0];
            string recordId = dtbl.Rows[0]["tbl_AssetAndGroupAccountId"].ToString();
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                string selectedValue;
                selectedValue = JsonConvert.SerializeObject(dtbl);
                audit_IUD.WebServiceSelectInsert("tbl_AssetAndGroupAccount", recordId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int DeleteAssetAndGroupAccount(AssetAndGroupAccountListUI assetAndGroupAccountListUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_AssetAndGroupAccount", assetAndGroupAccountListUI.Tbl_AssetAndGroupAccountId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
