...
/// </summary>
public class CustomerInvoiceListDAL : IRequiresSessionState
{
    ...
    public DataTable GetCustomerInvoiceListById(CustomerInvoiceListUI customerInvoiceListUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_CustomerInvoice", customerInvoiceListUI.Tbl_CustomerInvoiceId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public DataTable GetCustomerInvoiceListBySearchParameters(CustomerInvoiceListUI customerInvoiceListUI)
    {
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
                dtbl = ds.Tables[0];
            string recordId = dtbl.Rows[0]["tbl_CustomerInvoiceId"].ToString();
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                string selectedValue;
                selectedValue = JsonConvert.SerializeObject(dtbl);
                audit_IUD.WebServiceSelectInsert("tbl_CustomerInvoice", recordId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int DeleteCustomerInvoice(CustomerInvoiceListUI customerInvoiceListUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_CustomerInvoice", customerInvoiceListUI.Tbl_CustomerInvoiceId);
                }
