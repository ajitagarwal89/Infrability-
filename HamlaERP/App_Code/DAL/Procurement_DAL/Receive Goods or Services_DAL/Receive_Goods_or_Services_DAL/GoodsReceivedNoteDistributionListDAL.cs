...
/// </summary>
public class GoodsReceivedNoteDistributionListDAL : IRequiresSessionState
{
    ...
    public DataTable GetGoodsReceivedNoteDistributionListById(GoodsReceivedNoteDistributionListUI goodsReceivedNoteDistributionListUI)
    {
        ...
        try
        {
            ...
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                ...
                audit_IUD.WebServiceSelectInsert("tbl_GoodsReceivedNoteDistribution", goodsReceivedNoteDistributionListUI.Tbl_GoodsReceivedNoteDistributionId,selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public DataTable GetGoodsReceivedNoteDistributionListBySearchParameters(GoodsReceivedNoteDistributionListUI goodsReceivedNoteDistributionListUI)
    {
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
                dtbl = ds.Tables[0];
            string recordId = dtbl.Rows[0]["tbl_GoodsReceivedNoteDistributionId"].ToString();
            if (SessionContext.GlobalAuditEnabledStatus == true)
            {
                string selectedValue;
                selectedValue = JsonConvert.SerializeObject(dtbl);
                audit_IUD.WebServiceSelectInsert("tbl_GoodsReceivedNoteDistribution", recordId, selectedValue);
            }
        }
        catch (Exception exp)
        {
    ...
    public int DeleteGoodsReceivedNoteDistribution(GoodsReceivedNoteDistributionListUI goodsReceivedNoteDistributionListUI)
    {
        ...
        try
        {
            using (SqlConnection SupportCon = new SqlConnection(connectionString))
            {
                ...
                if (SessionContext.GlobalAuditEnabledStatus == true)
                {
                    audit_IUD.WebServiceDelete("tbl_GoodsReceivedNoteDistribution", goodsReceivedNoteDistributionListUI.Tbl_GoodsReceivedNoteDistributionId);
                }
                sqlCmd.Dispose();
                SupportCon.Close();
            }
